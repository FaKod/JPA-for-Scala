...
trait EntityManagerWrapper {
   ...
   * extended functionality of QueryWrapper, such as pagination or using
  ...
  /**
   * Creates a new QueryWrapper[A] using the given query string. The results
   ...
   * @return A new QueryWrapper[A] that uses the given query string
   */
  def createQuery[A](queryString: String) = new QueryWrapper[A](em.createQuery(queryString))

  /**
   * Creates a new QueryWrapper[A] using the given named query. The results
   * of the named query should be of type A.
   *
   * @param queryName The named query to use
   * @return A new QueryWrapper[A] that uses the given named query
   */
   ...
   * @return A new QueryWrapper[A] that uses the given native SQL
   */
  def createNativeQuery[A](sqlString: String, resultSetMapping: String) = new QueryWrapper[A](em.createNativeQuery(sqlString, resultSetMapping))

  /**
   * Closes the EntityManager. Subclasses may override this if they
   * desire different behavior.
   */
  //def close() = factory.closeEM(em) // @TODO change handling of factory
   ...
   */
  //def contains(entity: AnyRef) = em.contains(entity) //@TODO specs conflict
}
